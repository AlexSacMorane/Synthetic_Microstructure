...
area_pore = porosity * dim_sample**2
dim_pore = int((area_pore / np.pi)**0.5) # radius

# generate the pore (0)
for i_x in range(dim_sample):
    for i_y in range(dim_sample):
        if (i_x - dim_sample/2)**2 + (i_y - dim_sample/2)**2 < dim_pore**2:
            M_bin[i_x, i_y, :] = 0
...
with open('fft/cyl_incl/dict_fft_'+sample_id, 'wb') as handle:
...
write_vtk_structured_points('vtk/cyl_incl/cyl_incl_'+sample_id+'.vtk', Microstructure_vtk, spacing=(1.0, 1.0, 1.0), origin=(0, 0, 0), binary=False)  
