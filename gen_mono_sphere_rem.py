...
# init a counter
count = 0
# remove spheres randomly until the target porosity is reached
while 1-np.sum(M_bin)/(dim_sample**3) < porosity:
    ...
    count += 1
    if count % 500 == 0:
        print("Removed spheres:", count, " Current porosity:", round(1-np.sum(M_bin)/(dim_sample**3),2),'/', porosity)
    # create the sphere
...
with open('fft/mono_spheres_rem/dict_fft_'+sample_id, 'wb') as handle:
...
write_vtk_structured_points('vtk/mono_spheres_rem/mono_spheres_rem_'+sample_id+'.vtk', Microstructure_vtk, spacing=(1.0, 1.0, 1.0), origin=(0, 0, 0), binary=False)  
