...
# init a counter
count = 0
# insert spheres randomly until the target porosity is reached
while 1-np.sum(M_bin)/(dim_sample**3) > porosity:
    # generate a random sphere
    center = np.random.rand(3) * dim_sample
    radius = radius_sphere
    count += 1
    if count % 500 == 0:
        print("Inserted spheres:", count, " Current porosity:", round(1-np.sum(M_bin)/(dim_sample**3),2),'/', porosity)
    # create the sphere
    for i_x in range(int(center[0]-radius_sphere-dim_interface), int(center[0]+radius_sphere+dim_interface)):
        for i_y in range(int(center[1]-radius_sphere-dim_interface), int(center[1]+radius_sphere+dim_interface)):
            for i_z in range(int(center[2]-radius_sphere-dim_interface), int(center[2]+radius_sphere+dim_interface)):
                # compute the distance
                dist_sq = (i_x - center[0])**2 + (i_y - center[1])**2 + (i_z - center[2])**2
                if dist_sq < radius**2:
...
# compute the sdf of the binary array
...
with open('fft/mono_spheres_add_dict_fft_'+sample_id, 'wb') as handle:
...
write_vtk_structured_points('vtk/mono_spheres_add_'+sample_id+'.vtk', Microstructure_vtk, spacing=(1.0, 1.0, 1.0), origin=(0, 0, 0), binary=False)  
