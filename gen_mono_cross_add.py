...
# init a counter
count = 0
# insert crosses randomly until the target porosity is reached
while 1-np.sum(M_bin)/(dim_sample**3) > porosity:
    # generate a random cross
    position_000 = np.random.rand(3) * dim_sample
    count += 1
    if count % 500 == 0:
        print("Inserted crosses:", count, " Current porosity:", round(1-np.sum(M_bin)/(dim_sample**3),2),'/', porosity)
    # create the cross
    for i_x in range(int(position_000[0]), int(position_000[0]+size_cube)):
        for i_y in range(int(position_000[1])+size_cube, int(position_000[1]+2*size_cube)):
            for i_z in range(int(position_000[2])+size_cube, int(position_000[2]+2*size_cube)):
                ...
                # assign
                M_bin[i_x, i_y, i_z] = 1
    for i_x in range(int(position_000[0])+size_cube, int(position_000[0]+2*size_cube)):
        for i_y in range(int(position_000[1]), int(position_000[1]+size_cube)):
            for i_z in range(int(position_000[2])+size_cube, int(position_000[2]+2*size_cube)):
                # periodic condition (x-axis)
                if i_x >= dim_sample :
                    i_x = i_x - dim_sample
                ...
                M_bin[i_x, i_y, i_z] = 1
        for i_y in range(int(position_000[1])+size_cube, int(position_000[1]+2*size_cube)):
            for i_z in range(int(position_000[2]), int(position_000[2]+3*size_cube)):
                # periodic condition (x-axis)
                if i_x >= dim_sample :
                    i_x = i_x - dim_sample
                # periodic condition (y-axis)
                ...
                # assign
                M_bin[i_x, i_y, i_z] = 1
        for i_y in range(int(position_000[1])+2*size_cube, int(position_000[1]+3*size_cube)):
            for i_z in range(int(position_000[2])+size_cube, int(position_000[2]+2*size_cube)):
                # periodic condition (x-axis)
                if i_x >= dim_sample :
                    i_x = i_x - dim_sample
                ...
                # assign
                M_bin[i_x, i_y, i_z] = 1
    for i_x in range(int(position_000[0])+2*size_cube, int(position_000[0]+3*size_cube)):
        for i_y in range(int(position_000[1])+size_cube, int(position_000[1]+2*size_cube)):
            for i_z in range(int(position_000[2])+size_cube, int(position_000[2]+2*size_cube)):
                # periodic condition (x-axis)
                if i_x >= dim_sample :
...
with open('fft/mono_crosses_add/dict_fft_'+sample_id, 'wb') as handle:
...
write_vtk_structured_points('vtk/mono_crosses_add/mono_crosses_add_'+sample_id+'.vtk', Microstructure_vtk, spacing=(1.0, 1.0, 1.0), origin=(0, 0, 0), binary=False)  
