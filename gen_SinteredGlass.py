...
import skfmm, pickle, random, scipy
import numpy as np
...
# load dict
with open('SinteredGlass/dict_fft_'+namefile+'_255.dict', 'rb') as handle:
        dict_fft = pickle.load(handle)
M_bin = dict_fft['M_ct'] > 172
...
min_dim = min(M_bin.shape)

# random extraction
i_x_min = random.randint(0, M_bin.shape[0]-min_dim)
i_y_min = random.randint(0, M_bin.shape[1]-min_dim)
i_z_min = random.randint(0, M_bin.shape[2]-min_dim)

# extraction
M_bin = M_bin[i_x_min: i_x_min+min_dim, i_y_min: i_y_min+min_dim, i_z_min: i_z_min+min_dim].copy()
# new shape
print(M_bin.shape)
...
plt.savefig('SinteredGlass/plot_'+namefile+'.png')
plt.close()
...
plt.savefig('SinteredGlass/plot_cond_'+namefile+'.png')
plt.close()
